...
static void *virtual_output_create(obs_data_t *settings, obs_output_t *output)
{
	...
	data->hori_flip = obs_data_get_bool(settings, "mirror");
...
static bool virtual_output_start(void *data)
{
	...
	if (start) {
		...
		shared_queue_set_hori_flip(&out_data->video_queue, out_data->hori_flip);
...
static void virtual_video(void *param, struct video_data *frame)
{
	...
	if (out_data->hori_flip && out_data->flip_ctx.init) {
		...
		// shared_queue_push_video(&out_data->video_queue, (uint32_t*)out_data->flip_ctx.frame_out->linesize,
		//  out_data->width, out_data->height, out_data->flip_ctx.frame_out->data, frame->timestamp);
	...
	if (out_data->video_queue.header->connect_num > connect_num) {
		blog(LOG_INFO, "virtual camera has a new connect %d",out_data->video_queue.header->connect_num);
		connect_num = out_data->video_queue.header->connect_num;
		signal_handler_t *handler = obs_output_get_signal_handler(out_data->output);
		if (handler) {
...
{
	virtual_out_data *out_data = (virtual_out_data*)data;
	bool hori_flip = obs_data_get_bool(settings, "mirror");
	...
	if (hori_flip != out_data->hori_flip ||
		...
		shared_queue_set_hori_flip(&out_data->video_queue, hori_flip);
		pthread_mutex_unlock(&out_data->mutex);
	}
}
